...
- ✅ **ET accumulation**: Daily ET (transpiration + soil evap + canopy evap) is now accumulated in the hourly `lax.scan` loop and returned as `et_total` in `DailyOutput`. Validated by an ET non-negativity invariant test. Fixture-level ET validation remains deferred until the Fortran harness is extended to log daily ET.
- ✅ **JIT consistency test**: Verifies that `jax.jit(run_integration)` produces results matching the eager path to `rtol=1e-10`, guarding the compiled execution path that calibration workloads will rely on.
- ✅ **Grouped vmappable integration entrypoint**: Added `run_integration_grouped(...)` with public `IntegrationForcing` / `IntegrationParams` pytrees and kept the legacy flat `run_integration(...)` signature as a compatibility wrapper. A focused JAX test now verifies that two identical one-day sites can be evaluated via `jax.jit(jax.vmap(...))` and match the scalar grouped run. This is currently a JAX-side interface improvement; the `svmc-js` port is validated against the same replay fixture but does not yet expose an identical grouped public API or internal execution shape.
- ✅ **Lightweight grouped benchmark**: Added `scripts/benchmark_integration_compile.py` to measure compile+first-run and steady-state timings separately for the grouped integration entrypoint on a short Qvidja replay window. It now supports `--compile-only` and `--mode {single,batch,both}` so long GPU rollout probes can isolate compile cost without paying for repeated steady-state timing.
- **JAX structural debt reduction**: The direct Fortran-to-JAX port is numerically validated, but several boundary and trace-shape choices still create avoidable friction for JAX compilation and batching. This follow-up track is separate from numerical parity work and should remain behavior-preserving unless a reference discrepancy is found.
  1. ✅ **Hoist scalar wrapping out of scan bodies**: `_make_hourly_step` and `_make_daily_step` now hoist closure-constant scalars and small coefficient vectors out of the traced hourly/daily loops.
  2. ✅ **Replace traced Python loops in hot leaves**: `phydro.leaf_functions.viscosity_h2o` now uses array expressions instead of nested Python loops while preserving float64 fixture behavior.
  3. ✅ **Reduce tiny inner-loop array construction**: The hourly meteorology smoothing path now uses vectorized coefficients instead of rebuilding 2-element arrays from traced scalars.
  4. ✅ **Introduce vmappable integration parameter pytrees**: `run_integration` exposed a flat Fortran-shaped scalar signature. This is now addressed on the JAX side by `run_integration_grouped(...)`; remaining follow-up is to migrate downstream callers gradually where batching is valuable, without assuming the `svmc-js` port will keep an identical public shape.
  5. ✅ **Deduplicate AWENH matrix builders**: The repeated 5×5 AWENH assembly logic is now factored into a shared helper for `decompose`, `initialize_totc`, and `mod5c20`. Remaining follow-up is optional deeper consolidation of the distinct climate/size modifier calculations if that becomes a measurable maintenance burden.
  6. ✅ **Validate with narrow compile-sensitive tests first**: These cleanups were validated incrementally with narrow pytest slices before the full suite.
- **Management-regime coverage gap**: The 35-day integration window contains only one harvest event (day 34, `manage_type=1`). Grazing (`manage_type=2`) and organic amendment (`manage_type=3/4`) paths are not exercised at the integration level. Extending this requires either a longer fixture with real forcing that triggers those regimes, or synthetic single-day fixtures for each management type. Deferred to Phase 5 follow-up.
- ✅ **NaN fix in `canopy_water_snow`**: Canopy storage could go slightly negative (`-ε`) after evaporation, causing `0^{-0.4} = ∞` and `0 × ∞ = NaN` in the exposure coefficient on the next step. Fortran avoids this via short-circuit `if/else`; JAX evaluates all branches unconditionally. Fixed by clamping `w ≥ 0` and using `max(w, ε)` in the Ce formula. This eliminated NaN at day 269+ in full multi-year runs.
- ✅ **Stateful regression for guarded-formula hazards**: The existing branch audit already covered the explicit Fortran `canopy_water_snow` arms (phase, LAI guard, sublimation vs evaporation, unloading, interception, melt/freeze), but that audit is one-step and vendor-scoped. It did not catch the JAX-only failure mode where a prior timestep leaves canopy storage near zero and the next timestep evaluates a mathematically invalid guarded subexpression before masking it. Added targeted repeated cold/dry canopy tests so branch coverage claims are complemented by stateful regression coverage for JAX evaluation semantics.
- ✅ **1697-day Fortran vs JAX comparison**: `scripts/generate_comparison.py` runs both the Fortran harness and JAX integration for the full Qvidja dataset (1697 days, May 2018 – Dec 2022). All fields achieve R² ≥ 0.9999 over 4.6 years. Results written to `website/public/comparison-qvidja.json`; visualization at `website/comparison.html`.
- ✅ **Comparison website page**: `website/comparison.html` displays 8 overlay charts (GPP, NEE, SOC, soil moisture, carbon pools, Rh, ψ) with R² summary cards, plus LAI context and JAX-only ET. Linked from `website/index.html`.
